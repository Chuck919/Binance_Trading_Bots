...
bots_dict = {'grid_1': ['BTCUSDT', 25000, 30000, 10, 30]}
'''
...
class GridBot:
  ...
  def __init__(self, key, variables):
    ...
    self.low = variables[1]
    self.high = variables[2]
    self.grids = variables[3]
    self.total_budget = variables[4]
    self.use = variables[4] / variables[3]
    self.step = (self.high - self.low) / (self.grids - 1)
    self.orders = []
    self.profit = 0.0
    self.threshold = None
    if self.use / self.high < 0.0001:
      raise Exception('Too many grids')
    self.bought = 0
  ...
  def create_data(self):
    with sqlite3.connect('trading_data.db') as conn:
      cursor = conn.cursor()
      cursor.execute('SELECT bot_name FROM GridOrders WHERE bot_name = ?',
                     (self.key, ))
  ...
  def get_orders(self):
    with sqlite3.connect('trading_data.db') as conn:
      ...
      )  # Use 'fetchone' since there should be only one row
      return rows
  ...
  def place_buy_order(self, price):
    buy_order = client.create_order(
      ...
      quantity=Decimal(self.use / price).quantize(Decimal('0.0001'),
                                                  rounding=ROUND_DOWN),
      price=price)
  ...
  def cancel_orders(self, price):
    ...
    for i in self.orders:
      ...
      if order_info['status'] == 'NEW' or order_info[
  ...
  def place_orders(self):
    if self.bought >= 1:
      self.place_sell_order(
        float(
          Decimal(self.threshold + self.step).quantize(Decimal('0.01'),
                                                       rounding=ROUND_DOWN)))
    elif self.bought >= 2:
      self.place_sell_order(
        float(
          Decimal(self.threshold + 2 * self.step).quantize(
    ...
    self.place_buy_order(
      float(
        Decimal(self.threshold - 2 * self.step).quantize(Decimal('0.01'),
                                                         rounding=ROUND_DOWN)))
  ...
  def main(self, new_price):
    ...
    if new_price >= self.high or new_price <= self.low:
      ...
      buy_order = client.order_market_buy(
        symbol=self.symbol,
        quantity=float(
          Decimal(self.use / new_price).quantize(Decimal('0.0001'),
                                                 rounding=ROUND_DOWN)))
      self.bought += 1
      self.store_data()
    ...
    elif new_price > self.threshold + 2 * self.step and new_price < self.threshold + 3 * self.step:
      self.threshold += 2 * self.step
      self.cancel_orders(new_price)
      self.place_orders()
    ...
    elif new_price > self.threshold + 3 * self.step or new_price < self.threshold - 3 * self.step:
      self.threshold = None
      self.cancel_orders(new_price)
